...
const useBraintreeAchInstruments = (method: PaymentMethod) => {
    ...
    const isInstrumentFeatureAvailable = !customer?.isGuest && Boolean(method.config.isVaultingEnabled);
    ...
    const shouldConfirmInstrument = shouldShowInstrumentFieldset && !!currentInstrument && !currentInstrument.trustedShippingAddress;
