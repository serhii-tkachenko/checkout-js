...
import {PaymentFormValues, usePaymentFormContext} from '@bigcommerce/checkout/payment-integration-api';
...
const useBraintreeAchValidation = () => {
    ...
    const getValidationSchema = () => {
        ...
        const formFields = ownershipType === OwnershipTypes.Business
            ? businessBraintreeAchFormFields
            : personalBraintreeAchFormFields;
    ...
    const validateBraintreeAchForm = useCallback(async (braintreeAchFormValues: PaymentFormValues): Promise<boolean> => {
        const {
            businessName,
            accountNumber,
            routingNumber,
        ...
        const isValidDepositoryName = ownershipType === OwnershipTypes.Business
            ? isValidBusinessName
