...
interface BraintreeAcceleratedCheckoutInstrumentMenuItemProps {
    className?: string;
...
> = ({ className, instrument, testId, onClick }) => {
    ...
    return (
        <button className={className} data-test-id={testId} onClick={onClick} type="button">
            <div
            ...
            >
                ...
                <div className="instrumentSelect-card" data-test-id={`${testId ?? ''}-last4`}>
                    {cardInfo ? (
                        <TranslatedString
                            data={{
